...
void RichFeatureMatcher::MatchFeatures(int idx_i, int idx_j, vector<DMatch>* matches) {
	...
	stringstream ss; ss << "imgpts1 has " << imgpts1.size() << " points (descriptors " << descriptors_1.rows << ")" << endl;//stringstream ss;
    cout << ss.str();
...
//    double cutoff = 4.0*min_dist;
    std::set<int> existing_trainIdx; //Q
    for(unsigned int i = 0; i < matches->size(); i++ )
    { 
...
#ifdef __SFM__DEBUG__ 
    {
        ...
        resizeWindow("Good Matches",1200,800);
