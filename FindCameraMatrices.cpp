					  ...
					  ) 
{
	...
	if (matches.size() <= 0) { 
		...
		GetAlignedPointsFromMatch(imgpts1, imgpts2, matches, imgpts1_tmp, imgpts2_tmp);//Q
	}
	...
	Mat F;
	{
		...
		//pans0ul  but use imagepoints ,not imagepoint_good, to find F model is not proper
		F = findFundamentalMat(pts1, pts2, FM_RANSAC, 0.006 * maxVal, 0.99, status); //threshold from [Snavely07 4.1]
	}
			
	//pans0ul use imagepoint that may include wrong matches ,the result F will infulence next process .
	vector<DMatch> new_matches;
	cout << "F keeping " << countNonZero(status) << " / " << status.size() << endl;	
	for (unsigned int i=0; i<status.size(); i++) {
		if (status[i]) //fit F model 
		{
			imgpts1_good.push_back(imgpts1_tmp[i]);
			imgpts2_good.push_back(imgpts2_tmp[i]);
			if (matches.size() <= 0) { //points already aligned...//?
				new_matches.push_back(DMatch(matches[i].queryIdx,matches[i].trainIdx,matches[i].distance));//debug-
			} else {
